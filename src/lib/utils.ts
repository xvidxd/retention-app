...
import { Stream, ClientType } from "./types";

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

export function getClientTypeColor(type: ClientType | undefined | null): string {
  switch (type) {
    case 'A': return 'bg-green-100 text-green-800 border-green-200';
    case 'B': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    case 'C': return 'bg-red-100 text-red-800 border-red-200';
    case 'T': return 'bg-blue-100 text-blue-800 border-blue-200';
    case 'R': return 'bg-purple-100 text-purple-800 border-purple-200';
    case 'M': return 'bg-gray-200 text-gray-800 border-gray-300';
...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null, auth: any) {
  ...
  if (errInfo.error && errInfo.error.includes('Quota limit exceeded')) {
    console.warn('Firestore Quota Exceeded API limits. Please check Firebase limits.');
  } else {
    console.warn('Firestore Warning: ', errInfo.error);
  }
  // We remove the hard throw to prevent the app from ungracefully crashing 
  // on normal errors like quota limits.
