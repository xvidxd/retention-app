...
import { Check, X, UserCircle, ChevronLeft, ChevronRight, Send } from 'lucide-react';
...
export default function StudentProfile() {
    ...
    const unsubStudent = onSnapshot(doc(db, 'students', id), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (studentData.streamId) {
          ...
          unsubStream = onSnapshot(doc(db, 'streams', studentData.streamId), (streamSnap) => {
      ...
      handleFirestoreError(error, OperationType.GET, `students/${id}`, auth);
      setLoading(false);
    });
  ...
  const handleUpdateClientType = async (type: ClientType) => {
    ...
    try {
      ...
      logAudit('UPDATE_STUDENT', id, { field: 'clientType', value: type });
    } catch (error) {
  ...
  const handleUpdatePhone = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'students', id), {
        phone: newPhone || deleteField()
      });
      logAudit('UPDATE_STUDENT', id, { field: 'phone', value: newPhone });
  ...
  const handleUpdateTelegram = async () => {
    ...
    try {
      let newTelegram = telegramValue.trim();
      if (newTelegram && !newTelegram.startsWith('@')) {
        newTelegram = '@' + newTelegram;
      }
      await updateDoc(doc(db, 'students', id), {
        telegramUsername: newTelegram || deleteField()
      });
      logAudit('UPDATE_STUDENT', id, { field: 'telegramUsername', value: newTelegram });
  ...
  const handleUpdateStream = async (newStreamId: string) => {
    if (!id || !newStreamId) return;
    setUpdatingStream(true);
    try {
      const selectedStream = streams.find(s => s.id === newStreamId);
      const streamName = selectedStream?.name || '';
      await updateDoc(doc(db, 'students', id), {
        streamId: newStreamId,
        streamName
      });
      logAudit('UPDATE_STUDENT', id, { field: 'stream', value: streamName });
  ...
  const clientTypeColors = {
    ...
    M: 'bg-gray-200 text-gray-800 border-gray-300'
  };
        ...
        <div className="px-4 py-5 sm:px-6 flex justify-between items-center flex-wrap gap-4">
          <div className="flex items-center space-x-4">
            <div>
              ...
              <div className="mt-1 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                ...
                <div className="flex items-center gap-2 text-sm ml-0 sm:ml-4">
                  ...
                  {isEditingTelegram ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="border border-gray-300 rounded px-2 py-0.5 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none w-32"
                      ...
                      <button 
                        onClick={() => {
                          setIsEditingTelegram(false);
                          setTelegramValue(dynamicStudent.telegramUsername || '');
                        }}
                        className="text-red-600 hover:text-red-700"
                      >
                        <X className="w-4 h-4" />
                    ...
                    <div className="flex items-center gap-2 group">
                      <span className={dynamicStudent.telegramUsername ? "text-gray-900 font-medium" : "text-gray-400 italic"}>
                        {dynamicStudent.telegramUsername || 'нет'}
                      </span>
                      <button 
                        onClick={() => {
            ...
            {dynamicStudent.clientType && (
              <span className={`px-3 py-1 rounded-full text-sm font-bold border ${getClientTypeColor(dynamicStudent.clientType)}`}>
                {dynamicStudent.clientType}
              </span>
            ) || null}
          </div>
          <div className="flex items-center space-x-2 flex-wrap gap-y-2">
            <button
              ...
              className="mr-4 inline-flex items-center gap-1.5 px-3 py-1.5 border border-transparent text-xs font-medium rounded-md shadow-sm text-white bg-[#0088cc] hover:bg-[#0077b3]"
            >
              <Send className="w-3.5 h-3.5" />
              Написать в Telegram
            </button>
            <span className="text-xs text-gray-500 font-medium mr-2">Тип клиента:</span>
            {(['A', 'B', 'C', 'T', 'R', 'M'] as ClientType[]).map((type) => (
              <button
                key={type}
        ...
        <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl className="sm:divide-y sm:divide-gray-200">
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500 flex items-center">Поток</dt>
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                {isEditingStream ? (
                  <div className="flex items-center gap-2">
                    <select
                      value={streamValue || dynamicStudent.streamId}
                      onChange={(e) => setStreamValue(e.target.value)}
                      className="border border-gray-300 rounded px-2 py-1 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                    >
                      <option value="" disabled>Выберите поток</option>
                      {streams.slice().sort((a, b) => (a.name || '').localeCompare(b.name || '', undefined, { numeric: true })).map(s => (
                        <option key={s.id} value={s.id}>{s.name || s.id}</option>
                      ))}
                    </select>
                    <button 
                      onClick={() => handleUpdateStream(streamValue || dynamicStudent.streamId)}
                      disabled={updatingStream || (!streamValue && !dynamicStudent.streamId)}
                      className="text-green-600 hover:text-green-700 disabled:opacity-50 ml-1"
                    >
                      <Check className="w-4 h-4" />
                    </button>
                    <button 
                  ...
                  <div className="flex items-center gap-2 group">
                    <span>{dynamicStudent.streamName || 'Неизвестно'}</span>
                    <button 
                      onClick={() => {
                        setStreamValue(dynamicStudent.streamId);
