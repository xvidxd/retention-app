...
import { handleFirestoreError, OperationType, calculateStudentMetrics, getClientTypeColor } from '../lib/utils';
...
export default function Students() {
  ...
  const { students, streams, loading } = useData();
  ...
  const [bulkStatusType, setBulkStatusType] = useState<string>('');
  const bulkStatusFileInputRef = useRef<HTMLInputElement>(null);
  ...
  const processData = async (rows: any[], fileName: string) => {
    try {
      ...
      for (const row of rows) {
        ...
        const existingStudent = students.find(s => s.email?.toString()?.trim()?.toLowerCase() === email);
      ...
      logAudit('IMPORT_STUDENTS', null, { fileName, rowsCount: rows.length });
  ...
  const handleBulkStatusImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !bulkStatusType) return;

    setImporting(true);
    
    const processBulkData = async (rows: any[]) => {
      try {
        const batches: Promise<void>[] = [];
        let currentBatch = writeBatch(db);
        let operationCount = 0;
        ...
        for (const row of rows) {
          ...
          const student = students.find(s => s.email?.toString()?.trim()?.toLowerCase() === email);
          
          if (student) {
            const studentRef = doc(db, 'students', student.id);
            currentBatch.update(studentRef, { 
              clientType: bulkStatusType, 
        ...
        await Promise.all(batches);
        logAudit('BULK_UPDATE_STATUS', null, { targetStatus: bulkStatusType, parsedCount, updatedCount });
        setSuccessMessage(`Найдено почт в файле: ${parsedCount}. Успешно обновлено статусов: ${updatedCount}`);
      } catch (error) {
        console.error("Bulk status update error:", error);
        setErrorMessage('Ошибка массового обновления статусов.');
      } finally {
        setImporting(false);
        setShowBulkStatusModal(false);
        setBulkStatusType('');
        if (bulkStatusFileInputRef.current) bulkStatusFileInputRef.current.value = '';
      }
    };
  ...
  const confirmDeleteStudent = async () => {
    ...
    try {
      ...
      if (studentData) {
        logAudit('DELETE_STUDENT', studentToDelete, { email: studentData.email });
      }
  ...
  const handleSaveStudent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingStudent) {
        ...
        logAudit('UPDATE_STUDENT', editingStudent.id, { email, updates: studentData });
      } else {
        const docRef = await addDoc(collection(db, 'students'), { ...studentData, noMovementCount: 0 });
        logAudit('CREATE_STUDENT', docRef.id, { email, studentData });
      }
                      ...
                      {streamStudents.map(student => {
                        ...
                        <tr key={student.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap">
                            {student.clientType ? (
                              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border ${getClientTypeColor(student.clientType)}`}>
                                {student.clientType}
                              </span>
                            ) : (
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Загрузите Excel или CSV файл со столбцом <strong>Email</strong>.
              Всем найденным студентам будет проставлен выбранный тип клиента.
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Тип клиента</label>
                <select
                  value={bulkStatusType}
                  onChange={(e) => setBulkStatusType(e.target.value)}
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                >
                  <option value="" disabled>Выберите статус</option>
                  <option value="A">Тип А: Приоритет (актуально)</option>
                  <option value="B">Тип B: Мало времени (быт)</option>
                  <option value="C">Тип C: Забросили (не контактные)</option>
                  <option value="T">ТЕСТОВЫЙ: Админы и ИТ</option>
                  <option value="R">ВОЗВРАТ</option>
                  <option value="M">Мёртвые (М)</option>
                </select>
              </div>
              <div>
                <input
                  ...
                  accept=".xlsx, .xls, .csv"
                  className="hidden"
                  ref={bulkStatusFileInputRef}
                  onChange={handleBulkStatusImport}
                />
                <button
                  onClick={() => bulkStatusFileInputRef.current?.click()}
                  disabled={!bulkStatusType || importing}
                  className="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none disabled:opacity-50"
                >
                  {importing ? 'Обработка...' : 'Загрузить файл'}
                </button>
              </div>
            </div>
