...
import { collection, onSnapshot, query, setDoc, doc, getDoc, where } from 'firebase/firestore';
...
export default function Dashboard() {
  const { students, streams, loading: dataLoading } = useData();
  const [actions, setActions] = useState<Action[]>([]);
  const kpiChecked = useRef(false);
  ...
  useEffect(() => {
    ...
    // Only fetch actions that have a nextActionDate to drastically reduce Firestore reads
    const qActions = query(collection(db, 'actions'), where('nextActionDate', '>', ''));
  ...
  const targetStudents = dynamicStudents.filter(s => targetStreamIds.has(s.streamId) && s.clientType !== 'T' && s.clientType !== 'R' && s.clientType !== 'M');
  ...
  useEffect(() => {
    if (!auth.currentUser || loading || totalTarget === 0 || kpiChecked.current) return;
    
    const todayStr = new Date().toISOString().split('T')[0];
    
    const checkAndSaveKpi = async () => {
      try {
        const docRef = doc(db, 'kpi_history', todayStr);
        ...
        kpiChecked.current = true;
      } catch (error) {
        console.error("Error checking/saving KPI snapshot:", error);
      }
    };
    
    checkAndSaveKpi();
  }, [loading, totalTarget, startedPercent, progressingPercent, criticalCountTarget, droppedOutPercent]);

  const validStudents = dynamicStudents.filter(s => s.clientType !== 'T' && s.clientType !== 'R' && s.clientType !== 'M');
  
  const total = validStudents.length;
  const normal = validStudents.filter(s => s.status === 'normal').length;
  const lagging = validStudents.filter(s => s.status === 'lagging').length;
  const critical = validStudents.filter(s => s.status === 'critical').length;
  const notStarted = validStudents.filter(s => s.status === 'not_started').length;
  ...
  const priorityStudents = validStudents
  ...
  const pendingActions = actions
    .filter(a => a.nextActionDate && a.isCompleted === undefined)
    .filter(a => students.some(s => s.id === a.studentId))
    .sort((a, b) => {
                    ...
                    <li key={action.id} className="py-4">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center space-x-2 truncate pr-4">
                          {student?.clientType && (
                            <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold border ${getClientTypeColor(student.clientType)}`}>
                              {student.clientType}
                            </span>
                          )}
