...
import { collection, onSnapshot, query, setDoc, doc, addDoc, writeBatch, deleteDoc, updateDoc } from 'firebase/firestore';
...
export default function Streams() {
  ...
  const handleUpdateBlockDate = async (streamId: string, blockNum: number, dateStr: string) => {
    ...
    let updatedBlockDates = { ...stream.blockDates, [blockNum]: dateStr };

    // If updating block 1, automatically calculate subsequent blocks (every Wednesday)
    if (blockNum === 1 && dateStr) {
      const startDate = new Date(dateStr);
      for (let i = 2; i <= 13; i++) {
        const nextDate = new Date(startDate);
        nextDate.setDate(startDate.getDate() + (i - 1) * 7);
        updatedBlockDates[i] = nextDate.toISOString().split('T')[0];
      }
    }
  ...
  const executeBulkDelete = async () => {
    try {
      ...
      if (selectedStreamId && selectedForDeletion.has(selectedStreamId)) {
        ...
        <div className="col-span-1 bg-white shadow rounded-lg overflow-hidden flex flex-col">
          ...
          <ul className="divide-y divide-gray-200 overflow-y-auto max-h-[600px]">
            {streams.map((stream) => (
              <li 
                key={stream.id} 
                className={`p-4 cursor-pointer hover:bg-gray-50 flex items-center justify-between ${selectedStreamId === stream.id ? 'bg-indigo-50' : ''}`}
